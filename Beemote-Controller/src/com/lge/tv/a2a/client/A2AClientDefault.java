...
class A2AClientDefault extends A2AClient {
	...
	public boolean searchTV(Context context) {
		...
		WifiManager.MulticastLock multicastLock = wifiManager.createMulticastLock("A2AClientLock");
		...
		manager.setDiscoverable(new UDAPManager.Discoverable() {
			...
			public void OnSSDPPacket(DatagramPacket packet) {
				...
				URI location = URI.create(SSDP.parseHeaderValue(packet, "LOCATION"));
						...
						try {
							...
							while (eventType != XmlPullParser.END_DOCUMENT) {
								...
								case XmlPullParser.END_TAG:
									boolean isStart = eventType == XmlPullParser.START_TAG ? true : false;
								...
								case XmlPullParser.TEXT:
									if (inEnvelope) {
										...
										if (inPort) {
											tvinfo.port = Integer.parseInt(parser.getText());
										}
									}
									break;
								default:
							...
							if (eventListener != null) {
								handler.post(new Runnable() {
									...
									@Override
									public void run() {
										eventListener.onSearchEvent(tmpList, false);
									}
								});
							}
	...
	synchronized public A2ACmdError showPasscode() throws IOException {
		if (a2atvInfo != null) {
			int ret = UDAPManager.requestPairingStart(a2atvInfo.ipAddress, a2atvInfo.port, true);
	...
	synchronized public A2ACmdError hidePasscode() throws IOException {
		if (a2atvInfo != null) {
			int ret = UDAPManager.requestPairingStart(a2atvInfo.ipAddress, a2atvInfo.port, false);
			return parseError(ret);
		}
	...
	synchronized public A2ACmdError connect(String passcode) throws IOException {
		...
		if (a2atvInfo != null) {
			...
			registry.register("/udap/api/apptoapp/event/*", new HttpRequestHandler() {
					...
					if (request instanceof HttpEntityEnclosingRequest) {
						...
						if (entity != null) {
							...
							if (contents.indexOf(MAGIC_STR) > 0) {
								try {
									msgType = Integer.parseInt(contents.substring(0, contents.indexOf(MAGIC_STR)));
									contents = contents.substring(contents.indexOf(MAGIC_STR) + MAGIC_STR.length());
								} catch (Exception e) {
								}
							}

							if (messageListener != null) {
								messageListener.onRecieveMessage(msgType, contents);
							}
	...
	synchronized public A2ACmdError disconnect() throws IOException {
		if (a2atvInfo != null) {
			int ret = UDAPManager.requestPairingByebye(a2atvInfo.ipAddress, a2atvInfo.port,
	...
	synchronized public QueryResultAppID queryAppId(String appName) throws IOException {
		...
		if (a2atvInfo != null) {
			...
			try {
				URI uri = new URI("http://" + a2atvInfo.ipAddress + ":" + a2atvInfo.port + "/udap/api/apptoapp/data/"
				...
				if (statusCode == HttpURLConnection.HTTP_OK) {
					queryResultAppID.appId = Long.parseLong(EntityUtils.toString(entity));
				}
	...
	synchronized public QueryResultStatus queryAppStatus(long appId) throws IOException {
	...
	synchronized public A2ACmdError executeApp(long appId) throws IOException {
		if (a2atvInfo != null) {
			...
			try {
				URI uri = new URI("http://" + a2atvInfo.ipAddress + ":" + a2atvInfo.port
						+ "/udap/api/apptoapp/command/" + appId + "/run");
	...
	synchronized public A2ACmdError terminateApp(long appId) throws IOException {
		if (a2atvInfo != null) {
			...
			try {
				URI uri = new URI("http://" + a2atvInfo.ipAddress + ":" + a2atvInfo.port
						+ "/udap/api/apptoapp/command/" + appId + "/term");
	...
	synchronized public A2ACmdError sendMessage(long appId, int messageType, String message) throws IOException {
		if (a2atvInfo != null) {
			if (a2atvInfo.isConnected == false)
				return A2ACmdError.A2ACmdErrorUnauthorized;
			int statusCode = 0;
			...
			try {
				URI uri = new URI("http://" + a2atvInfo.ipAddress + ":" + a2atvInfo.port
						+ "/udap/api/apptoapp/command/" + appId + "/send");
				...
				if (messageType == 0)
					...
					StringEntity entity = new StringEntity(messageType + "~*(a)$^$(a)*~" + message, "UTF-8");
					Log.i("CSnopy", entity.getContentLength() + "," + messageType + "~*(a)$^$(a)*~" + message);
	...
	synchronized public A2ACmdError sendRemoteKey(int keyValue) throws IOException {
		if (a2atvInfo != null) {
			if (a2atvInfo.isConnected == false)
				return A2ACmdError.A2ACmdErrorUnauthorized;
			int statusCode = 0;
			...
			try {
				...
				{
					StringEntity entity = new StringEntity("<?xml version=\"1.0\" encoding=\"utf-8\"?><command><name>HandleKeyInput</name><value>"+keyValue+"</value></command>", "UTF-8");
